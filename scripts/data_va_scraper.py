#!/usr/bin/env python3
"""
Daily Data Analyst & Virtual Assistant Job Scraper
Searches 9 job sources for remote Data Analyst / Virtual Assistant roles and
emails a formatted digest to the configured recipient.

Sources
-------
  1. Indeed + ZipRecruiter   – via python-jobspy (LinkedIn/Glassdoor blocked)
  2. We Work Remotely        – RSS feeds for the relevant categories
...
SKILL_KEYWORDS = [
    ...
    # Data Analyst — languages & libraries
    "Python", "R", "pandas", "NumPy", "scipy", "statsmodels", "Jupyter",
    "tidyverse", "ggplot2", "dplyr",
    # Data Analyst — visualization
    "Tableau", "Power BI", "Looker", "Looker Studio", "Google Data Studio",
    "Metabase", "Mode", "Sigma", "Domo", "Qlik",
    # Data Analyst — spreadsheets
    "Excel", "Google Sheets", "VBA", "Pivot Table", "VLOOKUP",
    # Data Analyst — ETL & modeling
    "dbt", "Airflow", "Fivetran", "Stitch", "Hightouch",
    "ETL", "ELT", "Data Warehouse", "Data Modeling", "Star Schema",
    # Data Analyst — statistics & methods
    "A/B Testing", "Regression", "Forecasting", "Hypothesis Testing",
    "Statistical Analysis", "Cohort Analysis", "Funnel Analysis",
    "Customer Segmentation", "KPI",
    # Cloud (relevant to both)
    "AWS", "GCP", "Azure", "Athena",
    # Virtual Assistant — productivity suites
    "Google Workspace", "Microsoft 365", "Outlook", "Gmail",
    "Calendar Management", "Scheduling", "Email Management",
    # Virtual Assistant — communication
    "Slack", "Zoom", "Microsoft Teams", "Webex",
    # Virtual Assistant — project & task management
    "Notion", "Trello", "Asana", "ClickUp", "Monday.com", "Airtable",
    "Basecamp",
    # Virtual Assistant — CRM
    "HubSpot", "Salesforce", "Zoho", "Pipedrive", "CRM",
    # Virtual Assistant — bookkeeping
    "QuickBooks", "FreshBooks", "Xero", "Bookkeeping",
    # Virtual Assistant — design & marketing
    "Canva", "Mailchimp", "Constant Contact",
    "Buffer", "Hootsuite", "Later", "Sprout Social",
    # Soft skills that often appear in job descriptions
    "Customer Service", "Customer Support", "Data Entry",
    "Travel Planning", "Transcription", "Otter", "Grain",
]
...
_JOBRIGHT_TITLES = ",".join([
    ...
    "Financial Analyst", "Reporting Analyst", "Analytics Specialist",
...
_QA_TITLE_PATTERN = re.compile(
    r"\b("
    # Data Analyst family
    r"data analyst|senior data analyst|junior data analyst|lead data analyst|"
    r"business intelligence analyst|bi analyst|business analyst|"
    r"product analyst|marketing analyst|financial analyst|reporting analyst|"
    r"research analyst|operations analyst|customer analyst|people analyst|"
    r"insights analyst|analytics analyst|web analyst|"
    r"analytics specialist|analytics consultant|analytics engineer|"
    r"data analytics|data visualization|"
    # Virtual Assistant family
    r"virtual assistant|senior virtual assistant|"
    r"executive assistant|senior executive assistant|"
    r"administrative assistant|senior administrative assistant|"
    r"personal assistant|remote assistant|online assistant|"
    r"office assistant"
    r")\b",
    re.IGNORECASE,
)

# Block titles that look adjacent but are different roles
_TITLE_BLOCKLIST = re.compile(
    r"\b("
    r"sales|erp|backend developer|frontend developer|full.?stack developer|"
    r"call cent(re|er)|field engineer|instrumentation|"
    r"internal audit|"
    # Different specialization from Data Analyst
    r"data engineer|data scientist|machine learning|ml engineer|"
    r"devops engineer|site reliability engineer|sre|"
    # Engineering-adjacent assistant roles that aren't VAs
    r"research assistant|teaching assistant|lab assistant|graduate assistant|"
    r"medical assistant|dental assistant|nursing assistant|"
    r"physician assistant|clinical assistant|veterinary assistant|"
    # QA roles — handled by the other scraper
    r"qa engineer|qa analyst|qa automation|quality assurance|test engineer|sdet|"
    # Other roles we never want to surface here
    r"product manager|project manager|scrum master|"
    r"software engineer|account executive|account manager"
    r")\b",
    re.IGNORECASE,
)
...
# (Variable kept named `_CANONICAL_QA_TITLES` to minimise diff with the QA
#  scraper for easier future refactor into a shared profile module.)
_CANONICAL_QA_TITLES = [
    # Data Analyst family
    "data analyst", "senior data analyst", "junior data analyst",
    "lead data analyst", "business intelligence analyst", "bi analyst",
    "business analyst", "product analyst", "marketing analyst",
    "financial analyst", "reporting analyst", "research analyst",
    "operations analyst", "customer analyst", "people analyst",
    "insights analyst", "analytics analyst", "web analyst",
    "analytics specialist", "analytics consultant", "analytics engineer",
    "data analytics", "data visualization",
    # Virtual Assistant family
    "virtual assistant", "senior virtual assistant",
    "executive assistant", "senior executive assistant",
    "administrative assistant", "senior administrative assistant",
    "personal assistant", "remote assistant", "online assistant",
    "office assistant",
]
...
def _type_badges(job: dict) -> str:
    ...
    out = []
    # Role-family badge (DA vs VA)
    if any(k in title_l for k in ["analyst", "analytics", "insights"]):
        out.append('<span class="badge badge-blue">Data Analyst</span>')
    if "assistant" in title_l:
        out.append('<span class="badge badge-purple">Virtual Assistant</span>')
    # Skill-cluster badges
    if any(k in t for k in ["sql", "bigquery", "snowflake", "redshift", "databricks", "postgres", "mysql"]):
        out.append('<span class="badge badge-green">SQL / Data</span>')
    if any(k in t for k in ["tableau", "power bi", "looker", "metabase", "data studio", "qlik"]):
        out.append('<span class="badge badge-orange">Visualization</span>')
    if any(k in t for k in ["python", "pandas", "jupyter", " r ", " r,"]):
        out.append('<span class="badge badge-red">Python / R</span>')
    if any(k in t for k in ["dbt", "airflow", "etl", "elt", "fivetran", "stitch"]):
        out.append('<span class="badge badge-orange">ETL / Pipeline</span>')
    if any(k in t for k in ["a/b test", "regression", "forecasting", "hypothesis", "statistical"]):
        out.append('<span class="badge badge-red">Statistics</span>')
    if any(k in t for k in ["calendar", "scheduling", "email management", "inbox"]):
        out.append('<span class="badge badge-green">Admin / EA</span>')
    if any(k in t for k in ["customer service", "customer support", "helpdesk"]):
        out.append('<span class="badge badge-orange">Customer Service</span>')
    if any(k in t for k in ["social media", "mailchimp", "canva", "buffer", "hootsuite"]):
        out.append('<span class="badge badge-purple">Marketing / Social</span>')
    if any(k in t for k in ["quickbooks", "xero", "bookkeeping", "freshbooks"]):
        out.append('<span class="badge badge-green">Bookkeeping</span>')
    return "".join(out) or '<span class="badge badge-blue">DA / VA</span>'
...
def main() -> None:
    ...
    subject = f"[DA/VA Jobs] {len(qualified)} remote role(s) – {today}"
